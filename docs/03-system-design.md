...
- `DOCUMENT_MIN_SCORE` 設定一個 cosine 下限，預設 **0.35**。低於門檻的切塊不進結果。
...
**0.35 是暫定值。** 它是保守選的：目的是擋掉明顯講別件事的切塊，不是去否決一個本來就在正常工作的排序。目前的語料只有兩篇公告，而且調校需要能跑嵌入模型的機器，因此這個數字尚未用 50 題題庫實測過。關掉 gate 之前必須跑一次 `npm run evaluate` 確認：可回答的題目沒有被擋掉，Recall@5 維持 100%。語料擴充到 genshin-db 與 Fandom 之後，這個值需要重新量。
