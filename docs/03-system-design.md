...
- `DOCUMENT_MIN_SCORE` 設定一個 cosine 下限，預設 **0.47**。低於門檻的切塊不進結果。
...
**0.47 怎麼來的。** 用 bge-m3 對 50 題題庫實測後選的，不是猜的：

| 問題 | 最高相似度 |
|---|---:|
| `case:version-2-1-changes` | 0.532 |
| `case:version-5-0-changes` | 0.597 |
| `case:inazuma-new-islands` | 0.729 |
| `case:natlan-sub-regions` | 0.795 |
| 「納塔的火神是誰？」（語料答不出來） | **0.410** |

50 題裡只有 4 題真的走到文件檢索，其餘走結構化路線。可回答的最低分 0.532，語料答不出來的那題 0.410，0.47 落在這個間隙的中間，兩側各約 0.06 的餘裕。

實測結果（`DOCUMENT_MIN_SCORE=0.47`）：Recall@5 100%（40/40）、正確拒答率 100%（10/10）、非拒答附來源率 100%（40/40），40 題回答／10 題拒答的分布沒有改變。「納塔的火神是誰？」從「已回答＋1 筆引用」變成 `insufficient_evidence` 拒答、0 筆引用。

**但樣本只有 4 個正例。** 語料擴充到 genshin-db 與 Fandom 之後，走文件路線的題目會變多，這個值必須重新量。
