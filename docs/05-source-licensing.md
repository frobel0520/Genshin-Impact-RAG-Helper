...
> 狀態：**已決議 2026-09-02，採方案 A（只存指標，本機重建）**，由專案擁有者裁定。
> OPEN-06 已在 [`02-system-analysis.md`](02-system-analysis.md) 標記為已決議。
> 調查日期：2026-09-02。
...
release gate 的重現步驟、以及唯一一份真實語料都會消失，換來的是一個不能跑的 repo
與一份無法驗證的 gate 紀錄。

所以順序是：**先寫擷取器，再移除原文**，兩者在第二階段同一個 PR 裡完成。在那之前，
這兩個檔案是一個**有紀錄、有期限的例外**：

- 內容量：各約 500 字元，在一般引用範圍內，不構成 §5.3 所指的「大量」
- 期限：T36 第二階段完成時移除
- 這不是「以後再說」——它寫在這裡，第二階段的驗收條件會檢查它

`sources/README.md` 同理：現在改寫成擷取流程的說明，會描述一個還不存在的指令。
它跟著擷取器一起改。

## 6. 這份文件不是法律意見

以上是依公開條款整理的工程判斷，供專案擁有者決策使用；決策已於 2026-09-02 作出。
條款連結：
